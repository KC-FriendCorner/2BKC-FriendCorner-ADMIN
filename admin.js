 ...
 */
    ...
    let isSending = false; // ตัวแปรป้องกันการส่งซ้ำ

    function handleSendMessage() {
        // 1. ตรวจสอบว่ากำลังส่งอยู่หรือไม่ ถ้าใช่ให้หยุด (ป้องกันการรัวปุ่ม)
        if (isSending) return;

        const message = chatInput.value.trim();
        if (message !== "") {
            isSending = true; // ล็อกสถานะว่ากำลังส่ง

            if (typeof sendMessage === 'function') {
                sendMessage(); // เรียกฟังก์ชันส่งเดิมของคุณ

                // 2. ล้างค่าและรีเซ็ตความสูง
                chatInput.value = '';
                chatInput.style.height = 'auto';

                // 3. ปลดล็อกสถานะหลังจากส่งไปแล้ว 500ms (เสถียรที่สุดสำหรับมือถือ)
                setTimeout(() => {
                    isSending = false;
                }, 500);
            }
    ...
    // การจัดการ Event ในโทรศัพท์และคอมให้เสถียร
    if (chatInput) {
        // สำหรับกด Enter บนคีย์บอร์ด
        chatInput.addEventListener('keydown', function (e) {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                handleSendMessage();
        ...
        // สำหรับปุ่มส่ง (ใช้ click อย่างเดียวพอ ระบบจะจัดการ touch ให้เอง)
        if (sendButton) {
